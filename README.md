# Hi there, I'm Mohab Ahmed 👋

### Junior Data Analyst | Cairo, Egypt

Passionate about turning raw data into meaningful insights. I've worked with real-world datasets spanning retail sales, e-commerce, and business analytics — building dashboards, data models, and end-to-end pipelines.

- 🔭 Currently working on data analysis projects using Python, Power BI, and SQL
- 🌱 Pursuing the IBM Data Analyst Professional Certificate (Coursera)
- 💼 Former Data Analyst Intern Semicolon Codes
- 📫 Reach me at: mohab.ahmed.ntss@gmail.com

---

## 🛠️ Tools & Technologies

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-CC2927?style=flat&logo=microsoftsqlserver&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat&logo=powerbi&logoColor=black)
![Excel](https://img.shields.io/badge/Excel-217346?style=flat&logo=microsoftexcel&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat&logo=streamlit&logoColor=white)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=flat&logo=plotly&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat&logo=python&logoColor=white)
![Seaborn](https://img.shields.io/badge/Seaborn-3C7EBF?style=flat&logo=python&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
---

## 🚀 Featured Projects

### [ShopNile — End-to-End Data Analysis](https://github.com/Mohab209/ShopNile-Analysis-Using-Python)
> Python · Pandas · Streamlit · Plotly

Built a full data pipeline from raw messy data to a live interactive dashboard. Covers 4 datasets (500 customers, 120 products, 3,000 orders, 3,050 payments), applied comprehensive data cleaning, and deployed publicly on Streamlit Cloud.

---

### [MB1 Company Sales Analysis](https://github.com/Mohab209/MB1-Company-Dashboard-Analysis)
> Excel · Power Pivot · DAX

Analyzed $629.6M in net sales across a star schema model with 5 interrelated tables. Built an interactive Excel dashboard with dynamic slicers, KPI cards, and 4 chart types revealing Electronics as the dominant category (64% of profit).

---

### [AdventureWorks Sales & Product Analytics](https://github.com/Mohab209/Sales-Products-Analytics-Project)
> Power BI · Power Query · DAX

Identified peak sales months and flagged underperforming periods through time series analysis. Delivered geospatial and product-level insights via an interactive Power BI dashboard.

---

### [Store Database — SQL Analysis](https://github.com/Mohab209/SQL-Analysis-Project)
> Microsoft SQL Server

Designed and implemented a normalized relational database with 4 tables. Wrote queries using JOINs, aggregations, and GROUP BY to analyze product performance and calculate total revenue.

---

## 📊 GitHub Stats

![Profile Views](https://komarev.com/ghpvc/?username=Mohab209&label=Profile+Views&color=0e75b6&style=flat)
[![GitHub followers](https://img.shields.io/github/followers/Mohab209?label=Followers&style=flat&color=0e75b6)](https://github.com/Mohab209)
[![GitHub stars](https://img.shields.io/github/stars/Mohab209?label=Total+Stars&style=flat&color=f1c40f)](https://github.com/Mohab209)

| Metric | Value |
|---|---|
| 💻 Focus Area | Data Analysis & Visualization |
| 🛠️ Primary Tools | Python · Power BI · SQL · Excel |
| 📁 Projects | 11 end-to-end analysis projects |
| 🎓 Certifications | 3 completed · 1 in progress |

---

## 🎓 Certifications

| Certificate | Provider | Date |
|---|---|---|
| Data Analyst in Power BI | DataCamp | Apr 2026 |
| Data Analysis with Excel Power Tools | DataCamp | Jan 2026 |
| Excel Fundamentals | DataCamp | Dec 2025 |
| IBM Data Analyst Professional Certificate | Coursera & IBM | In Progress |

---

## 📬 Connect with me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mohabhigazy)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:mohab.ahmed.ntss@gmail.com)
